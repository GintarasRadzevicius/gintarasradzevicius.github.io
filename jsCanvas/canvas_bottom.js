...
function LineBottom(){
    ...
    this.createNewLine = function(){
        ...
        let arrCoords = getDataBottom(Math.random() * window.innerWidth, window.innerHeight, hypotenuse, speed);
    ...
    this.update = function(){
        ...
        if(this.boolLineStartsAtBottomLeftSide){
            ...
            if(this.y1 < window.innerHeight){                                                            //line starts from minus X coord axis, therefore shrink should start after line breaks x coord axis
...
function getDataBottom(x1, y1, hypotenuse, speed){                      //The function moves the line below x axis, so it is not visible
    let x2 = 0;
    let y2 = 0;
    
    let angleTriangleSideOnYCoord = getAngleOnYBottom(x1);
    ...
    if(x1 < window.innerWidth/2){
        ...
        } while (window.innerHeight + maxLength> y2);
    } else{
        do {                                                    //for line start from below of screen
            x1 = x1 + differenceSideX;
            x2 = x2 + differenceSideX;
